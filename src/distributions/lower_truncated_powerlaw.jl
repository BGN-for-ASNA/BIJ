"""
    bi_dist_lower_truncated_power_law(; alpha, low, validate_args=nothing, name="x", obs=nothing, mask=nothing, sample=false, seed=nothing, shape=(), event=0, create_obj=false, to_jax=true)
...
- `shape`: A numeric vector: A multi-purpose argument for shaping. When `sample=False` (model building), this is used
...
`
"""
function bi_dist_lower_truncated_power_law(; 
    alpha,
    low,
    ...
    return BI_INSTANCE[].dist.lower_truncated_power_law(
        alpha=jnp_alpha,
        low=jnp_low,
