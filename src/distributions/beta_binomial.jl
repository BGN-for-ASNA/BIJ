"""
    bi_dist_beta_binomial(; concentration1, concentration0, total_count=1, validate_args=nothing, name="x", obs=nothing, mask=nothing, sample=false, seed=nothing, shape=(), event=0, create_obj=false, to_jax=true)
...
- `concentration1`: A numeric vector, matrix, or array representing the first concentration parameter (alpha) of the Beta distribution.
- `concentration0`: A numeric vector, matrix, or array representing the second concentration parameter (beta) of the Beta distribution.
- `total_count`: A numeric vector, matrix, or array representing the number of Bernoulli trials in the Binomial part of the distribution.
...
`
"""
function bi_dist_beta_binomial(; 
    concentration1,
    concentration0,
    total_count=1,
    ...
    return BI_INSTANCE[].dist.beta_binomial(
