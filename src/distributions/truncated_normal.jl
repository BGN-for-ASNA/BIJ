"""
    bi_dist_truncated_normal(; loc=0.0, scale=1.0, low=nothing, high=nothing, validate_args=nothing, name="x", obs=nothing, mask=nothing, sample=false, seed=nothing, shape=(), event=0, create_obj=false, to_jax=true)
...
`
"""
function bi_dist_truncated_normal(; 
    loc=0.0,
    scale=1.0,
    low=nothing,
    high=nothing,
    ...
    return BI_INSTANCE[].dist.truncated_normal(
        loc=jnp_loc,
        scale=jnp_scale,
        low=jnp_low,
        high=jnp_high,
