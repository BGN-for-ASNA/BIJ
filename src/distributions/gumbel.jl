"""
    bi_dist_gumbel(; loc=0.0, scale=1.0, validate_args=nothing, name="x", obs=nothing, mask=nothing, sample=false, seed=nothing, shape=(), event=0, create_obj=false, to_jax=true)
...
- `shape`: A numeric vector. When \code{sample=FALSE} (model building), this is used with \code{.expand(shape)} to set the distribution's batch shape. When \code{sample=TRUE} (direct sampling), this is used as \code{sample_shape} to draw a raw JAX array of the given shape.
...
- `create_obj`: If \code{TRUE}, returns the raw BI distribution object instead of creating a sample site.
...
function bi_dist_gumbel(; 
    ...
    return BI_INSTANCE[].dist.gumbel(
        loc=jnp_loc,
        scale=jnp_scale,
