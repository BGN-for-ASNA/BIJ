"""
    bi_dist_multinomial(; total_count=1, probs=nothing, logits=nothing, total_count_max=nothing, validate_args=nothing, name="x", obs=nothing, mask=nothing, sample=false, seed=nothing, shape=(), event=0, create_obj=false, to_jax=true)
...
- `shape`: A numeric vector used for shaping. When \code{sample=FALSE} (model building), this is used with `.expand(shape)` to set the distribution's batch shape. When \code{sample=TRUE} (direct sampling), this is used as `sample_shape` to draw a raw JAX array of the given shape.
...
`
"""
function bi_dist_multinomial(; 
    total_count=1,
    probs=nothing,
    logits=nothing,
    total_count_max=nothing,
    ...
    jnp_total_count_max = JNP[].array(total_count_max)

    return BI_INSTANCE[].dist.multinomial(
        total_count=jnp_total_count,
        probs=jnp_probs,
        logits=jnp_logits,
        total_count_max=jnp_total_count_max,
