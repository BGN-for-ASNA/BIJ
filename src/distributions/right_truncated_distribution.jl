"""
    bi_dist_right_truncated_distribution(; base_dist, high=0.0, validate_args=nothing, name="x", obs=nothing, mask=nothing, sample=false, seed=nothing, shape=(), event=0, create_obj=false, to_jax=true)
...
Equation: `f_`\text`trunc``(x) = \frac`f_`\text`base``(x)``F_`\text`base``(\text`high`)` \quad \text`for ` x \le \text`high``
- `base_dist`: The base distribution to truncate. Must be a univariate distribution with real support.
- `high`: (float, jnp.ndarray, optional): The upper truncation point. The support of the new distribution is \eqn{(-\infty, \text{high})}. Defaults to 0.0.
...
`
"""
function bi_dist_right_truncated_distribution(; 
    base_dist,
    high=0.0,
    ...
    return BI_INSTANCE[].dist.right_truncated_distribution(
