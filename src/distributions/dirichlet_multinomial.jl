"""
    bi_dist_dirichlet_multinomial(; concentration, total_count=1, validate_args=nothing, name="x", obs=nothing, mask=nothing, sample=false, seed=nothing, shape=(), event=0, create_obj=false, to_jax=true)
...
`
"""
function bi_dist_dirichlet_multinomial(; 
    concentration,
    total_count=1,
    ...
    return BI_INSTANCE[].dist.dirichlet_multinomial(
