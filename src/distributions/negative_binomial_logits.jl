"""
    bi_dist_negative_binomial_logits(; total_count, logits, validate_args=nothing, name="x", obs=nothing, mask=nothing, sample=false, seed=nothing, shape=(), event=0, create_obj=false, to_jax=true)
...
- `logits`: A numeric vector, matrix, or array representing the log-odds parameter.
Related to the probability of success.
- `shape`: A numeric vector. A multi-purpose argument for shaping. When `sample=False`
...
`
"""
function bi_dist_negative_binomial_logits(; 
    total_count,
    logits,
    ...
    return BI_INSTANCE[].dist.negative_binomial_logits(
