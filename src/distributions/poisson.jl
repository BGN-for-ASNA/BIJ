"""
    bi_dist_poisson(; rate, is_sparse=false, validate_args=nothing, name="x", obs=nothing, mask=nothing, sample=false, seed=nothing, shape=(), event=0, create_obj=false, to_jax=true)
...
Equation: ` \mathrm`rate`^k \frac`e^`-\mathrm`rate````k!``
...
function bi_dist_poisson(; 
    ...
    return BI_INSTANCE[].dist.poisson(
        rate=jnp_rate,
