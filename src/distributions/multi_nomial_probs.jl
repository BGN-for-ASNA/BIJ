"""
    bi_dist_multinomial_probs(; probs, total_count=1, total_count_max=nothing, validate_args=nothing, name="x", obs=nothing, mask=nothing, sample=false, seed=nothing, shape=(), event=0, create_obj=false, to_jax=true)
...
Equation: `P(X = x) = \frac`n!``x_1! x_2! \cdots x_k!` p_1^`x_1` p_2^`x_2` \cdots p_k^`x_k``
where:
...
`
"""
function bi_dist_multinomial_probs(; 
    probs,
    total_count=1,
    ...
    return BI_INSTANCE[].dist.multinomial_probs(
        probs=jnp_probs,
