"""
    bi_dist_bernoulli(; probs=nothing, logits=nothing, validate_args=nothing, name="x", obs=nothing, mask=nothing, sample=false, seed=nothing, shape=(), event=0, create_obj=false, to_jax=true)
...
- `shape`: A numeric vector specifying the shape of the output.  Used with `.expand(shape)` when `sample=False` (model building) to set the distribution's batch shape. When `sample=True` (direct sampling), this is used as `sample_shape` to draw a raw JAX array of the given shape.
...
function bi_dist_bernoulli(; 
    ...
    return BI_INSTANCE[].dist.bernoulli(
        probs=jnp_probs,
        logits=jnp_logits,
