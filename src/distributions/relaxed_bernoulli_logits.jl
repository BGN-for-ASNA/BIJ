"""
    bi_dist_relaxed_bernoulli_logits(; temperature, logits, validate_args=nothing, name="x", obs=nothing, mask=nothing, sample=false, seed=nothing, shape=(), event=0, create_obj=false, to_jax=true)
...
Equation: `P(x) = \sigma\left(\frac`x``\text`temperature``\right)`
...
- `shape`: A numeric vector specifying the shape of the distribution.  When `sample=False` (model building),
...
`
"""
function bi_dist_relaxed_bernoulli_logits(; 
    temperature,
    logits,
    ...
    return BI_INSTANCE[].dist.relaxed_bernoulli_logits(
