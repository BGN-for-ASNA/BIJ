"""
    bi_dist_truncated_cauchy(; loc=0.0, scale=1.0, low=nothing, high=nothing, validate_args=nothing, name="x", obs=nothing, mask=nothing, sample=false, seed=nothing, shape=(), event=0, create_obj=false, to_jax=true)
...
Equation: `f_`\text`trunc``(x) = \frac`f_`\text`base``(x)``F_`\text`base``(\text`high`) - F_`\text`base``(\text`low`)` \quad \text`for ` \text`low` \le x \le \text`high``
...
`
"""
function bi_dist_truncated_cauchy(; 
    loc=0.0,
    scale=1.0,
    low=nothing,
    high=nothing,
    ...
    return BI_INSTANCE[].dist.truncated_cauchy(
        loc=jnp_loc,
        scale=jnp_scale,
        low=jnp_low,
        high=jnp_high,
