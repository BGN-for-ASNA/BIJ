"""
    bi_dist_laplace(; loc=0.0, scale=1.0, validate_args=nothing, name="x", obs=nothing, mask=nothing, sample=false, seed=nothing, shape=(), event=0, create_obj=false, to_jax=true)

Laplace Distribution
Samples from a Levy distribution.
The Lévy distribution is a continuous probability distribution on the positive real line (or shifted positive line) that is *heavy-tailed*, *skewed*, and arises naturally in connection with stable distributions (specifically the case with stability index Equation: `\alpha = \tfrac12`.
It is often used in contexts such as hitting-time problems for Brownian motion, physics (e.g., van der Waals line-shapes), and modelling very heavy-tailed phenomena. Let (X) be a Lévy-distributed random variable with **location** parameter $\mu$ and **scale** parameter (c > 0). The support is Equation: `x \ge \mu`.
...
- `create_obj`: Logical; If TRUE, returns the raw BI distribution object instead of creating a
...
function bi_dist_laplace(; 
    ...
    return BI_INSTANCE[].dist.laplace(
        loc=jnp_loc,
        scale=jnp_scale,
