"""
    bi_dist_projected_normal(; concentration, validate_args=nothing, name="x", obs=nothing, mask=nothing, sample=false, seed=nothing, shape=(), event=0, create_obj=false, to_jax=true)
...
Equation: `p(x) = \frac`1``Z` \exp\left(-\frac`1``2\sigma^2` ||x - \mu||^2\right)`
- `concentration`: A numeric vector representing the concentration parameter,
representing the direction towards which the samples are concentrated.
- `shape`: A numeric vector used for shaping. When \code{sample=FALSE} (model
...
- `create_obj`: Logical; If `TRUE`, returns the raw BI distribution
...
`
"""
function bi_dist_projected_normal(; 
    concentration,
    ...
    return BI_INSTANCE[].dist.projected_normal(
