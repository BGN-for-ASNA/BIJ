"""
    bi_dist_zero_sum_normal(; scale, event_shape, validate_args=nothing, name="x", obs=nothing, mask=nothing, sample=false, seed=nothing, shape=(), event=0, create_obj=false, to_jax=true)
...
- `scale`: A numeric vector or array representing the standard deviation of the underlying normal distribution before the zerosum constraint is enforced.
- `event_shape`: (numeric vector): The shape of the event, defining the dimensions of the vector that will be constrained to sum to zero.
...
`
"""
function bi_dist_zero_sum_normal(; 
    scale,
    event_shape,
    ...
    jnp_event_shape = JNP[].array(event_shape)

    return BI_INSTANCE[].dist.zero_sum_normal(
        scale=jnp_scale,
        event_shape=jnp_event_shape,
