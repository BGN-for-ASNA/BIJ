"""
    bi_dist_half_cauchy(; scale=1.0, validate_args=nothing, name="x", obs=nothing, mask=nothing, sample=false, seed=nothing, shape=(), event=0, create_obj=false, to_jax=true)
...
function bi_dist_half_cauchy(; 
    ...
    return BI_INSTANCE[].dist.half_cauchy(
        scale=jnp_scale,
