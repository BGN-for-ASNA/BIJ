"""
    bi_dist_discrete_uniform(; low=0, high=1, validate_args=nothing, name="x", obs=nothing, mask=nothing, sample=false, seed=nothing, shape=(), event=0, create_obj=false, to_jax=true)
...
Equation: `P(X = k) = \frac`1``high - low + 1`, \quad k \in \`low, low+1, ..., high\``
DiscreteUniform
Samples from a Discrete Uniform distribution.
...
`
"""
function bi_dist_discrete_uniform(; 
    low=0,
    high=1,
    ...
    return BI_INSTANCE[].dist.discrete_uniform(
        low=jnp_low,
        high=jnp_high,
